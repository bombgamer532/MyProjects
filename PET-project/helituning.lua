...
RegisterNetEvent('updHeliTuning')
AddEventHandler('updHeliTuning', function(data)
	CreateThread(function()
		while not helituning do
			Wait(0)
		end
		while #helituning == 0 do
			Wait(0)
		end
		for i=1,#helituning,1 do
			while #helituning[i] == 0 do
				Wait(0)
			end
		end
		for i=1,#data,1 do
			for j=1,#data[i],1 do
				helituning[i][j] = tonumber(data[i][j])
			end
		end
	end)
end)
...
local SaveTuning = function()
	...
	for i=1,#helis,1 do
		for j=1,#helituning[i],1 do
			data[i][j] = helituning[i][j]
		end
	end
	TriggerServerEvent('saveHeliTuning', data)
end
	...
	while true do
		...
		if(not IsCarDead(heli)) then
			for j=1,#helispecs,1 do
				if(GetCarModel(heli, _i) == helispecs[j][2]) then
					tempcar = j
				end
			end
		end
...
CreateThread(function()
	while true do
		...
		if(not DoesBlipExist(tuningblip)) then
			tuningblip = AddBlipForCoord(-267.25989, 215.44418, 222.59259, _i)
			ChangeBlipSprite(tuningblip, 75)
			ChangeBlipScale(tuningblip, 0.7)
			ChangeBlipNameFromAscii(tuningblip, "Helicopter tuning shop")
			ChangeBlipColour(tuningblip, 19)
			SetBlipAsShortRange(tuningblip, true)
		end
		DrawTextAtCoord(-267.25989, 215.44418, 222.59259, "Helicopter_tuning_shop", 20)
		DrawCheckpointWithDist(-267.25989, 215.44418, 222.59259-1, 1.1, 255, 255, 0, 100)
		if(IsPlayerNearCoords(-267.25989, 215.44418, 222.59259, 10)) then
			if(IsCharInAnyCar(GetPlayerChar(GetPlayerId()))) then
				if(GetDriverOfCar(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), _i) == GetPlayerChar(GetPlayerId())) then
					...
					if(IsGameKeyboardKeyJustPressed(18)) then
						...
						SetCarCoordinates(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), -267.25989, 215.44418, 222.59259-1)
						SetCarHeading(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), 179.714614868164)
						...
						local tempprice = 0
						if(IsCharInCar(GetPlayerChar(GetPlayerId()), heli)) then
							for j=1,#helispecs,1 do
								if(GetCarModel(heli, _i) == helispecs[j][2]) then
									tempcar = j
								end
							end
							if(helituning[tempcar][21] > 0) then
						...
						if(menuresult > 0) then
							...
							elseif(tempitems[menuresult] == "Customize") then
								if(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i) == heli) then
									for j=1,#helispecs,1 do
										if(GetCarModel(heli, _i) == helispecs[j][2]) then
											tempcar = j
										end
									...
									c = {}
									for j=1,4,1 do
										c[j] = helituning[tempcar][j]
									end
									for j=1,3,1 do
										rgb[j] = helituning[tempcar][13+j]
									end
									::customize::
									local amount = 0
									for j=1,4,1 do
										if(helituning[tempcar][j] ~= c[j]) then
											amount = amount + 1
										end
									end
									for j=1,9,1 do
										if(IsVehicleExtraTurnedOn(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), j)) then
											if(helituning[tempcar][j+4] ~= 1) then
												amount = amount + 1
											end
										else
											if(helituning[tempcar][j+4] ~= 0) then
												amount = amount + 1
											end
										end
									end
									for j=1,3,1 do
										if(helituning[tempcar][13+j] ~= rgb[j]) then
											amount = amount + 1
										end
									end
									local tempitems = {}
									...
									if(menuresult > 0) then
										if(tempitems[menuresult] == "Purchase ~g~(" .. 200*amount .. "$)") then
											if(money >= 200*amount) then
												...
												for j=1,4,1 do
													helituning[tempcar][j] = c[j]
												end
												for j=1,9,1 do
													if(IsVehicleExtraTurnedOn(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), j)) then
														helituning[tempcar][j+4] = 1
													else
														helituning[tempcar][j+4] = 0
													end
												end
												for j=1,3,1 do
													helituning[tempcar][13+j] = rgb[j]
												end
												SaveTuning()
							...
							elseif(tempitems[menuresult] == "Upgrade") then
								if(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i) == heli) then
									...
									for j=1,#tempprices,1 do
										if(j <= 3) then
											if(helituning[tempcar][16+j] ~= 3) then
												tempupg[j] = "" .. helituning[tempcar][16+j] .. " -> " .. helituning[tempcar][16+j]+1 .. " ~g~(" .. tempprices[j][helituning[tempcar][16+j]+1] .. "$)"
											else
												tempupg[j] = "3 (MAX)"
											end
										else
											if(helituning[tempcar][16+j] == 0) then
									...
									if(menuresult > 0) then
										...
										if(menuresult <= 3) then
											if(helituning[tempcar][16+menuresult] ~= 3) then
												if(money >= tempprices[menuresult][helituning[tempcar][16+menuresult]+1]) then
													money = money - tempprices[menuresult][helituning[tempcar][16+menuresult]+1]
													SaveStats()
													helituning[tempcar][16+menuresult] = helituning[tempcar][16+menuresult] + 1
													SaveTuning()
											...
											if(level >= 30) then
												if(helituning[tempcar][16+menuresult] == 0) then
													if(money >= tempprices[menuresult]) then
														money = money - tempprices[menuresult]
														SaveStats()
														helituning[tempcar][16+menuresult] = 1
														SaveTuning()
														TriggerEvent('chatMessage', '[Repair shop]', {0, 255, 0}, 'Upgrade installed')
													else
														TriggerEvent('chatMessage', '[Repair shop]', {255, 0, 0}, 'You cannot afford it!')
							...
							if(IsCharInAnyCar(GetPlayerChar(GetPlayerId()))) then
								...
								if(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i) == heli) then
									for j=1,#helispecs,1 do
										if(GetCarModel(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), _i) == helispecs[j][2]) then
											tempcar = j
										end
									end
								end
								if(tempcar > 0) then
									ChangeCarColour(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), helituning[tempcar][1], helituning[tempcar][2])
									SetExtraCarColours(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), helituning[tempcar][3], helituning[tempcar][4])
									for j=1,9,1 do
										if(helituning[tempcar][j+4] == 0) then
											TurnOffVehicleExtra(GetCarCharIsUsing(GetPlayerChar(GetPlayerId()), _i), j, true)
										else

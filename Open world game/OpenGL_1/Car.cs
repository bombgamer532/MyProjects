...
namespace OpenGL_1
{
    public class Car : Vehicle
    {
        ...
        public Car(float x, float y, float z, float ry)
        {
            ...
            vehcomp.Add(new VehComp("cylinder", 30, 20, 60, 0, 90, 0, 20, 20, 20, 0, 0, 0));
            vehcomp.Add(new VehComp("cylinder", -30, 20, 60, 0, -90, 0, 20, 20, 20, 0, 0, 0));
            vehcomp.Add(new VehComp("cylinder", 30, 20, -60, 0, 90, 0, 20, 20, 20, 0, 0, 0));
            vehcomp.Add(new VehComp("cylinder", -30, 20, -60, 0, -90, 0, 20, 20, 20, 0, 0, 0));
            vehcomp.Add(new VehComp("cube", 0, 30, 0, 0, 0, 0, 90, 30, 200, color1[0], color1[1], color1[2]));
            vehcomp.Add(new VehComp("cube", 0, 55, 30, -30, 0, 0, 85, 35, 20, 100, 100, 255));
            vehcomp.Add(new VehComp("cube", 0, 55, -30, 30, 0, 0, 85, 35, 20, 100, 100, 255));
            vehcomp.Add(new VehComp("cube", 0, 60, 0, 0, 0, 0, 90, 30, 55, color1[0], color1[1], color1[2]));
			this.x = x;
            this.y = y;
            this.z = z;
            this.ry = ry;
        ...
        public static void CarBehavior(object sender, EventArgs e)
        {
            var vehs = All;
            for (int j = 0; j < vehs.Count; j++)
            {
                ...
                if (GetDistanceBetweenCoords(player.x, player.y, player.z, vehs[j].x, vehs[j].y, vehs[j].z) < 2500)
                {
                    if (GetDistanceBetweenCoords(player.x, player.y, player.z, vehs[j].x, vehs[j].y, vehs[j].z) < 2000)
                    {
                        vehs[j].collision = true;
                    }
                    else
                    {
                        vehs[j].collision = false;
                    }
                    if (vehs[j].type != "dead")
                    {
                        if (vehs[j] is Car)
                        {
                            var car = (Car)vehs[j];
                            if (car.user != null && car.user != player)
                            {
                                if (car.node == -1)
                                {
                                    car.node = GetClosestCoordFromCoordsById(new Point(car.x, car.y, car.z), carnodes);
                                }
                                else
                                {
                                    car.ry = GetHeading(car.x, car.z, carnodes[car.node].x, carnodes[car.node].z);
                                    car.speed = 1;
                                    car.x += car.speed * dt * Sin(car.ry);
                                    car.z += car.speed * dt * Cos(car.ry);
                                    if (GetDistanceBetweenCoords(car.x, car.y, car.z, carnodes[car.node].x, carnodes[car.node].y, carnodes[car.node].z) < 20)
                                    {
                                        if (links[car.node].Length > 1)
                                        {
                                            Random rnd = new Random();
                                            int r = rnd.Next(0, links[car.node].Length);
                                            while (links[car.node][r] == car.lastnode)
                                            {
                                                r = rnd.Next(0, links[car.node].Length);
                                            }
                                            car.lastnode = car.node;
                                            car.node = links[car.lastnode][r];
                                        }
                                        else
                                        {
                                            car.lastnode = car.node;
                                            car.node = links[car.lastnode][0];
                                        }
                                    }
                                }
                            }
